...
const sampleTemplate = `
	...
	{{range $index, $item := .Items}}
		{{$index}}: {{$item}}
	{{end}}
	We can also easily import other functions like strings.Splie
	them immediately used the array created as a result:
...
func RunTemplate() error {
	data := struct {
		...
		OtherVariable: "I'm defind in a second template!",
	}
	...
	// 두 번째 템필릿을 파싱(parsing)해 또 다른 템플릿을 생성한다.
	t2, err := t.Clone()
	if err != nil {
		return err
	}
