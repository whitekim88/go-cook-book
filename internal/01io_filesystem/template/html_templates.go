...
// HTMLDifferences 함수는 html/template과 /text/template의 차이점을 강조한다
func HTMLDifferences() error {
	...
	// 자동 회피(탈출, escape) 처리한다. 이는 상황에 따라 인식되며
	// 변수가 렌더링되는 위치에 따라 다르게 동작한다.
	err = t.Execute(os.Stdout, map[string]string{"Name": "<script>alert('you have been pwned')</script>"})
	if err != nil {
		return err
	}

	// 수동으로 Escapter를 호출할 수도 있다.
